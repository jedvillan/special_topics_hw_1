...
const publishAsync = (
  ...
  let publishDelayMs = 10000; // 10 seconds interval
  if (shouldBackoff) {
  ...
  setTimeout(() => {
    ...
    const payload = JSON.stringify({ "ts":ts, "temperature":temp, "humidity":humidity });
...
const mqttDeviceDemo = (
  ...
  //const mqttTopic = 'projects/specialtopics-290508/subscriptions/gcf-save_data_to_firestore-us-west2-rasppi4-cpu-temp';
...
const publishAsyncGateway = (
  ...
  setTimeout(() => {
    ...
    const schedulePublishDelayMs = 6000; // messageType === 'events' ? 1000 : 2000;
    setTimeout(() => {
      const secsFromIssue = parseInt(Date.now() / 1000) - iatTime;
